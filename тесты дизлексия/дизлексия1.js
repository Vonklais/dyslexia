...
function createFullTimeline() {
    ...
    const totalDuration = config.exposureTime + config.interStimulusInterval; // Полное время триала
    ...
    const trainingTest = {
        timeline: [{
            ...
            stimulus_duration: config.exposureTime, // Стимул скрывается после exposureTime
            data: {
            ...
            on_finish: function(data) {
                const prevTrial = jsPsych.data.get().last(2).values()[1]; // Предыдущий триал
                const currentTime = performance.now();

                if (data.response === " ") {
                    const rt = data.rt;
                    if (prevTrial && prevTrial.is_target && !data.is_target && rt > config.exposureTime) {
                        prevTrial.reaction_time = currentTime - prevTrial.stimulus_onset;
                        prevTrial.correct = true;
                        data.is_response_to_previous = true;
                        data.correct = null;
                        ...
                        console.log("Ответ на текущий:", { is_target: data.is_target, correct: data.correct, rt: rt });
                    }
                } else {
                    data.correct = data.is_target ? false : true;
                    data.reaction_time = data.is_target ? config.exposureTime : null;
                    console.log("Нет ответа:", { is_target: data.is_target, correct: data.correct, rt: data.reaction_time });
                }
            }
        }],
    ...
    timeline.push({
        ...
        stimulus: function() {
            ...
            let correctResponses = trials.filter({ correct: true }).count();
            ...
            console.log("Training results:", { correctResponses, totalTargets, accuracy, trials: trials.values() });
            return `
        ...
        timeline: [
            ...
            },
            {
                ...
                stimulus: function() {
                    ...
                    console.log("Main results:", { correctResponses, totalTargets, accuracy, trials: trials.values() });
                    return `
                        <p>Тест завершён.</p>
                        <p>Правильные ответы: ${correctResponses} / ${totalTargets} (${accuracy}%)</p>
                        <p>Среднее время реакции: ${avgRt} мс</p>
