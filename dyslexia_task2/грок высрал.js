...
const experimentState = {
    trainingAccuracy: 0,
    targetWord: null,
    trainingStimuliData: null, // Стимулы для тренировки
    mainStimuliData: null,     // Стимулы для основного этапа
    trainingIteration: 0       // Счетчик итераций тренировки
};
...
function createFullTimeline() {
    ...
    // Функция для генерации тренировочных стимулов
    function generateTrainingStimuli() {
        const trainingStimuliCount = Math.floor(myDict.trainingTime / totalDuration);
        return wordLength === 3 ? generateStimuli3(trainingStimuliCount, letters) : generateStimuli4(trainingStimuliCount, letters);
    }

    // Тренировочный этап с повторением
    const trainingBlock = {
        timeline: [
            {
                ...
                stimulus: function() {
                    return `
                        <p>Тренировочный этап (${myDict.trainingTime / 1000} секунд, попытка ${experimentState.trainingIteration + 1}).</p>
                ...
                post_trial_gap: 1000,
                on_start: function() {
                    experimentState.trainingIteration++; // Увеличиваем счетчик перед началом каждой тренировки
                }
            },
            {
                timeline: [{
                    ...
                    data: {
                        ...
                        phase: "training",
                        prev_correct: false,
                        training_iteration: experimentState.trainingIteration // Записываем номер итерации
                    },
                    on_start: function(data) {
                        data.stimulus_onset = performance.now();
                    },
                    on_finish: function(data) {
                        const allTrials = jsPsych.data.get().filter({ phase: "training", training_iteration: experimentState.trainingIteration }).values();
                ...
                timeline_variables: generateTrainingStimuli().stimuli, // Генерируем стимулы напрямую
                randomize_order: false
            },
            {
                type: jsPsychHtmlKeyboardResponse,
                stimulus: function() {
                    let trials = jsPsych.data.get().filter({ phase: "training", training_iteration: experimentState.trainingIteration });
                    ...
                    console.log("Training results (iteration " + experimentState.trainingIteration + "):", { correctResponses, prevCorrect, allCorrectResponses, totalTargets, accuracy });
                    return `
                        <p>Тренировка завершена (попытка ${experimentState.trainingIteration}).</p>
                        <p>Точность: ${accuracy}% (${allCorrectResponses} из ${totalTargets})</p>
                        <p>Среднее время реакции: ${avgRt} мс</p>
                        <p>${accuracy >= 30 ? 'Вы прошли тренировку! Нажмите клавишу для основного теста.' : 'Точность ниже 30%. Нажмите клавишу, чтобы повторить тренировку.'}</p>
                    `;
                }
            }
        ],
        loop_function: function() {
            // Повторяем тренировку, если точность < 30%
            return experimentState.trainingAccuracy < 30;
        }
    };

    // Добавляем тренировочный блок в таймлайн
    timeline.push(trainingBlock);

    // Основной этап
    const mainStimuliCount = Math.floor(myDict.testTime / totalDuration);
    experimentState.mainStimuliData = wordLength === 3 ? generateStimuli3(mainStimuliCount, letters) : generateStimuli4(mainStimuliCount, letters);
    ...
    timeline.push({
        timeline: [
            ...
            },
            {
                ...
                timeline_variables: experimentState.mainStimuliData.stimuli, // Используем mainStimuliData
                randomize_order: false
            },
            {
                type: jsPsychHtmlKeyboardResponse,
